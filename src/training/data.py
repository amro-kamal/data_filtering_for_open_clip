...
def get_dataset_size(shards, prune_ratio=1.0, dataset_size=437444349):
    ...
    # LAION-440M: 438358791 #437444349

    if prune_ratio < 1.0:
        logging.info(
            f"Total Dataset size: {dataset_size}. Using {prune_ratio*100}% of the data ({int(prune_ratio*dataset_size)}) examples"
        )
    else:
        logging.info(f"Total Dataset size: {dataset_size}")

    total_size = int(prune_ratio * dataset_size)
...
def log_and_continue(exn):
    ...
    logging.warning(
        f"(local rank, global rank, world_size):{world_info_from_env()}: Handling webdataset error ({repr(exn)}). Ignoring."
    )
    return True
...
def trie_search(trie, seq="<shard>/<tar>/<key>", prune_ratio=1.0):
    '''
    Search for example id (String) in a trie file.
    '''
    try:
        if trie[seq]:
            res = True
...
def tar_file_iterator(
    ...
    for tarinfo in stream:
        ...
        try:
            ...
            if trie is not None:
                # -- "trie" is float for random sampling with replacement after each epoch.
                # -- "trie" is pygtrie.StringTrie for filtering/pruning.

                # -- Skip the example id its id exists (or doesn't exists) in the trie file
                ...
                # else:
                #     print("✅✅ take ", file_key, url, fname) ## -- Use this line for debugging


            ## -- Read the file here (if you pass from the if statement above)
...
def tar_file_expander(data, trie, prune_ratio, handler=reraise_exception):
    ...
    for source in data:
        ...
        try:
            ...
            for sample in tar_file_iterator(
                source["stream"], url, trie=trie, prune_ratio=prune_ratio
            ):
...
def tarfile_to_samples_nothrow(src, trie, prune_ratio, handler=log_and_continue):
    """
    streams_of_tar_files = url_opener ----> tar_file_expander(streams) -----> tar_file_iterator() -------> group_by_keys()
    """
    streams = url_opener(src, handler=handler)
    files = tar_file_expander(
        streams, trie=trie, prune_ratio=prune_ratio, handler=handler
    )
    samples = group_by_keys_nothrow(files, handler=handler)
...
def tarfile_to_samples(src, trie, prune_ratio, handler=log_and_continue):
    """
    streams_of_tar_files = url_opener ----> tar_file_expander(streams) -----> tar_file_iterator() -------> group_by_keys()
    """
    streams = url_opener(src, handler=handler)
...
def get_wds_dataset(
    ...
    num_samples, num_shards = get_dataset_size(
        input_shards, args.prune_ratio, args.train_num_samples
    )

    assert isinstance(
        args.prune_ratio, float
    ), f"prune_ratio should be of type float not {type(args.prune_ratio)}"
    if args.subset_file == "":
        assert args.prune_ratio == 1.0
        trie = None
        logging.info(f"Training on 100% of the data ({num_samples}) examples.")

    elif args.subset_file.lower() == "dynamic":
        trie = args.prune_ratio
        logging.info("Using Dynamic Random Pruning")
        print(
            f"Dynamic Rondom pruning with ratio of {args.prune_ratio}, using {num_samples} images..."
        )
        ...
        with open(args.subset_file, "rb") as f:
            ...
            ), "The object stored in the subset_file is not pygtrie object"
            logging.info(
